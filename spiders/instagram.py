...
class InstagramSpider(scrapy.Spider):
    ...
    # usernames_to_parse = ['skills_lovers']  # Пользователь, у которого собираются посты
    usernames_to_parse = ['qpaya__', 'mr_loser_0']  # Пользователь, у которого собираются посты
    # usernames_to_parse = ['photo.me', 'datascience_enthusiast']  # Пользователь, у которого собираем посты
    # usernames_to_parse = ['ai_machine_learning', 'learn.machinelearning', 'python.learning']  # Пользователь, у которого собираются посты
    ...
    def parse(self, response: HtmlResponse):  # Первый запрос на стартовую страницу
        csrf_token = self.fetch_csrf_token(response.text)  # csrf token берется из html
    ...
    def parse_user_data(self, response: HtmlResponse, username):
        user_id = self.fetch_user_id(response.text, username)  # Получение id пользователя

        variables = {                                          # Формирование словаря для передачи даных в запрос
            "id": user_id,
            "include_reel": True,
            "fetch_mutual": False,
            "first": 12                                        # 12 постов (макс. 50)
        }
    ...
    def parse_user_info(self, response: HtmlResponse, username, user_id, variables, followed_by):  # Прием ответа. Параметры от cb_kwargs
        ...
        for user in users:  # Перебор подписчиков, сбор данных
